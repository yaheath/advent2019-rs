...
fn part1(input: &ProgMem) -> i64 {
    ...
    vm.run_with_cb(&mut || 1, &mut output).unwrap();
    last_output
}
...
fn part2(input: &ProgMem) -> i64 {
    ...
    let mut vm = IntcodeVM::with_mem(&input);
    let mut output = |v| { last_output = v; };
    vm.run_with_cb(&mut || 5, &mut output).unwrap();
    last_output
}
...
mod tests {
    ...
    fn day05_test() {
        ...
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 8, &mut |v| assert_eq!(v, 1)).unwrap();
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 2, &mut |v| assert_eq!(v, 0)).unwrap();

        let input: Vec<ProgMem> = test_input("3,9,7,9,10,9,4,9,99,-1,8");
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 8, &mut |v| assert_eq!(v, 0)).unwrap();
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 2, &mut |v| assert_eq!(v, 1)).unwrap();
        ...
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 0, &mut |v| assert_eq!(v, 0)).unwrap();
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 2, &mut |v| assert_eq!(v, 1)).unwrap();
        ...
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || -1, &mut |v| assert_eq!(v, 999)).unwrap();
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 8, &mut |v| assert_eq!(v, 1000)).unwrap();
        let mut vm = IntcodeVM::with_mem(&input[0]);
        vm.run_with_cb(&mut || 80, &mut |v| assert_eq!(v, 1001)).unwrap();
    }
}
